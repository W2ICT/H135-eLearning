...
function exportExamPDF() {
  ...
  // ── หน้าข้อสอบ (10 ข้อ/หน้า) ──
  let qPages = '';
  const perPage = 10;
  for (let p = 0; p < Math.ceil(qs.length / perPage); p++) {
    const chunk = qs.slice(p * perPage, p * perPage + perPage);
    qPages += `<div class="print-page">
      ...
      <div class="print-meta">
        <span>หน้า ${p + 1} / ${Math.ceil(qs.length / perPage)}</span>
        <span>Name: ________________________________  Score: _______ / 100</span>
      </div>
      ${chunk.map((q, j) => {
        const n = p * perPage + j + 1;
        const userIdx = ans[n - 1];
        return `<div class="print-q">
          <div class="print-q-num">Q${n}. [Ch ${q._ch.num}] ${done && userIdx !== undefined ? (userIdx === q.a ? '<span style="color:green">✓</span>' : '<span style="color:red">✗</span>') : ''}</div>
