...
// Real training-manual images loaded from manifest.json (built by extract_images.mjs)
let PHOTO_MANIFEST = {};
fetch('images/manifest.json')
  .then(r => r.json())
  .then(data => { PHOTO_MANIFEST = data; })
  .catch(() => { /* no manifest — images simply won't show */ });
...
function openChapter(id) {
  ...
  // Real training-manual photo gallery
  const photos = PHOTO_MANIFEST[ch.num] || [];
  if (photos.length > 0) {
    const cards = photos.map((p, i) => `
      <div class="img-card" onclick="openLightbox('${p.file}', ${i}, ${photos.length})">
        <img src="${p.file}" alt="Training manual reference image ${i+1}" loading="lazy">
        <div class="img-caption">📄 Reference ${i+1} — PDF p.${p.page}</div>
      </div>`).join('');
    sectionsHtml += `
      <div class="cs">
        <h3><span>📷</span> Reference Images — Training Manual</h3>
        <p style="font-size:13px;color:var(--txt3);margin-bottom:14px">Extracted directly from the official Airbus EC135 P3H/T3H Pilot Training Manual PDF. Click any image to expand.</p>
        <div class="img-gallery">${cards}</div>
      </div>`;
  }

  document.getElementById('chSections').innerHTML = sectionsHtml;
...
function openLightbox(src, idx, total) {
  _lbIdx = idx || 0;
  _lbPhotos = [];
  // Gather all images from current chapter for navigation
  const ch = CHAPTERS[state.currentChapter];
  if (ch) {
    const photos = PHOTO_MANIFEST[ch.num] || [];
    _lbPhotos = photos.map(p => p.file);
  }
  if (_lbPhotos.length === 0) _lbPhotos = [src];
