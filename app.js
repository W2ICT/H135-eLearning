...
async function exportExamPDF() {
  ...
  // ── Loading state on all Export buttons ──
  ...
  const qCards = qs.map((q, i) => {
    ...
    const opts = q.o.map((opt, k) => {
      let s = 'display:flex;gap:6px;align-items:flex-start;padding:3px 6px;border-radius:3px;font-size:11.5px;margin-bottom:2px;';
    ...
    const expHtml = (done && q.e)
      ? `<div style="margin-top:6px;padding:5px 8px;background:#e8f4fd;border-left:3px solid #1565c0;font-size:10.5px;color:#333;line-height:1.45;border-radius:2px">💡 ${q.e}${q.ref ? ` <i style="color:#777"> · ${q.ref}</i>` : ''}</div>`
      : '';

    const label = done ? (isOk ? ' <span style="color:#155724;font-weight:700">✓ ถูก</span>' : ' <span style="color:#721c24;font-weight:700">✗ ผิด</span>') : '';

    return `<div style="border:1px solid ${border};border-left:4px solid ${border};border-radius:5px;padding:10px 12px;margin-bottom:9px;background:${bg};page-break-inside:avoid;">
      <div style="font-size:9.5px;color:#666;margin-bottom:3px">Q${i+1} · Ch ${q._ch.num} — ${q._ch.title}${label}</div>
      <div style="font-size:12.5px;font-weight:600;color:#111;margin-bottom:7px;line-height:1.45">${q.q}</div>
      ${opts}${expHtml}
    </div>`;
  }).join('');

  // ── Build: Answer key grid (10 columns) ──
  const keyGrid = qs.map((q, i) => {
    ...
    return `<div style="border:1px solid ${border};border-radius:3px;padding:4px 2px;text-align:center;background:${bg};font-family:sans-serif">
      <div style="font-size:8.5px;color:#888">Q${i+1}</div>
      <div style="font-size:12px;font-weight:700;color:#1a237e">${L[q.a]}</div>
      ${done && uIdx !== undefined ? `<div style="font-size:8.5px;color:${isOk?'#155724':'#721c24'};font-weight:700">${L[uIdx]}</div>` : '<div style="font-size:8.5px;color:#ccc">—</div>'}
    </div>`;
  }).join('');

  // ── Score badge ──
  const scoreBadge = done
    ? `<span style="display:inline-block;margin-left:12px;padding:4px 16px;border-radius:20px;
        background:${pct>=60?'#d4edda':'#f8d7da'};border:1px solid ${pct>=60?'#2e7d32':'#c62828'};
        font-size:13px;font-weight:700;color:${pct>=60?'#155724':'#721c24'}">
        ${correct} / ${qs.length} &nbsp;(${pct}%) &nbsp;— ${pct>=60?'PASS ✓':'FAIL ✗'}
       </span>` : '';

  // ── Assemble full document element ──
  const el = document.createElement('div');
  el.style.cssText = 'width:190mm;font-family:Segoe UI,Arial,sans-serif;background:#fff;color:#000;padding:0;';
  el.innerHTML = `
    <!-- HEADER -->
    <div style="text-align:center;padding-bottom:12px;margin-bottom:14px;border-bottom:2.5px solid #1a237e;">
      <div style="font-size:8px;font-weight:700;color:#1a237e;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:4px">
        Airbus Helicopters Deutschland GmbH · For Instruction Only
      </div>
      <h1 style="font-size:19px;font-weight:900;color:#1a237e;margin:0 0 3px">H135 Type Training — แบบทดสอบ 100 ข้อ</h1>
      <p style="font-size:11px;color:#555;margin:0">EC135 P3H/T3H · Pilot Type Course · ${date}</p>
      <div style="margin-top:8px">${scoreBadge}</div>
    </div>
    <!-- META -->
    <div style="display:flex;justify-content:space-between;font-size:10px;color:#777;margin-bottom:12px;padding-bottom:6px;border-bottom:1px solid #eee;">
      <span>ทั้งหมด ${qs.length} ข้อ &nbsp;·&nbsp; 20 บท &nbsp;·&nbsp; Mixed Level</span>
      <span>Name: _________________________________&nbsp;&nbsp;Score: _______ / ${qs.length}</span>
    </div>
    <!-- QUESTIONS -->
    ${qCards}
    <!-- ANSWER KEY PAGE BREAK -->
    <div style="page-break-before:always;padding-top:14px;">
      <h2 style="font-size:14px;font-weight:700;color:#1a237e;margin-bottom:4px;">
        Answer Key — ตารางเฉลย
      </h2>
      ${done ? '<p style="font-size:10px;color:#555;margin:0 0 10px">สีเขียว=ถูก &nbsp;·&nbsp; สีแดง=ผิด &nbsp;·&nbsp; บน=เฉลย &nbsp;·&nbsp; ล่าง=คำตอบของคุณ</p>' : '<p style="font-size:10px;color:#555;margin:0 0 10px">แสดงเฉพาะเฉลย (ยังไม่ได้ส่งคำตอบ)</p>'}
      <div style="display:grid;grid-template-columns:repeat(10,1fr);gap:3px;margin-bottom:0;">${keyGrid}</div>
    </div>`;

  // ── Loading overlay (covers screen so user sees progress) ──
  const overlay = document.createElement('div');
  overlay.style.cssText = [
    'position:fixed', 'inset:0', 'background:rgba(7,13,26,0.93)',
    'z-index:99999', 'display:flex', 'align-items:center',
    'justify-content:center', 'flex-direction:column', 'gap:14px'
  ].join(';');
  overlay.innerHTML = `
    <div style="font-size:40px">📄</div>
    <div style="color:#a78bfa;font-size:16px;font-weight:700">กำลังสร้าง PDF…</div>
    <div style="color:#94a3b8;font-size:12px">ทั้ง 100 ข้อ · กรุณารอสักครู่</div>`;
  document.body.appendChild(overlay);

  // Position el in-viewport so html2canvas can render it (NOT off-screen)
  ...
  // Give browser one frame to paint before html2canvas captures
  await new Promise(r => setTimeout(r, 200));

  try {
    await html2pdf().set({
      margin:       [8, 8, 8, 8],
      filename:     `H135_Exam_${new Date().toISOString().slice(0,10)}.pdf`,
      image:        { type: 'jpeg', quality: 0.97 },
      html2canvas:  { scale: 2, useCORS: true, logging: false, backgroundColor: '#ffffff', windowWidth: el.scrollWidth },
